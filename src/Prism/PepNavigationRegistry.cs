...
using Navigation = Prism.Navigation.Xaml.Navigation;

namespace PrismExperiment.Prism;

public class PepNavigationRegistry : PepViewRegistryBase, INavigationRegistry
{
    public PepNavigationRegistry(IEnumerable<ViewRegistration> registrations)
    ...
    private static void ConfigurePage(IContainerProvider container, Page page)
    {
        ...
        if (Navigation.GetContainerProvider((BindableObject)page) is null)
            Navigation.SetContainerProvider((BindableObject)page, container);
